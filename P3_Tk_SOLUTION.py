...
#<FONT>
buttonFont = tkfont.Font(family="fipps", size=int(HEIGHT/1000*18))
optionFont = tkfont.Font(family="fipps", size=int(HEIGHT/1000*12))
choiceFont = tkfont.Font(family="fipps", size = int(HEIGHT/1000*20))
textFont = tkfont.Font(family="fipps", size = int(HEIGHT/1000*15))
titleFont = tkfont.Font(family="fipps", size = int(HEIGHT/1000*25))
#<DEFAULT_SETTING>
...
def createButtonW(cat):
    ...
    if cat == "Insurance":
        ...
        label=Label(back, text="""
        This insurance level cover
        accidents such as rusty
        nail, broken arm, vanda-
        lism, etc.""", bg=bgc)
        ...
        label=Label(back, text="""
        This insurance level cover
        accidents such as blizzards,
        earthquakes, tornado, etc in
        addition to what plan 1
        covers.""", bg=bgc)

        label.place(x=375, y=350)
...
#Next Turn
def eventsWindow(cat):
    #create the window
    nw = Toplevel(mw)
    nw.geometry(str(int(HEIGHT/1000 * 1000))+"x"+str(int(HEIGHT/1000 * 600))+"+0+"+str(int(HEIGHT/1000*480)))
    nw.resizable(1, 1)
    nw.overrideredirect(True)   
    nw.title(cat)
    ...
    qb = Button(qbf, text="Back", font = optionFont, bg=brown,fg=white, 
            activebackground=actb, activeforeground=actf,
        command=lambda: nw.destroy())
    qbf.place(x=HEIGHT/1000*800, y=HEIGHT/1000*550)
    qb.pack(fill=BOTH, expand=1)
    ...
    def start(frame, mn):
        if Current_State.d['profile'][0] == "":
            label = Label(frame, text="You need to choose a class first", bg=bgc)
            label.pack()
        else:
            mn.destroy()
    ...
    if cat == "Start":
        qb.destroy()
        winTitle.destroy()
        qb = Button(qbf, text="Start", font = optionFont, bg=brown,fg=white, 
                    activebackground=actb, activeforeground=actf,
                    command=lambda: start(qbf, nw))
        qb.pack(fill=BOTH, expand=1)

        label1 = Label(back, text="The Divide", font=textFont, bg=bgc)
        label1.pack()
        
        fl2 = Frame(height=int(HEIGHT/1000*450), width=int(HEIGHT/1000*800), master=back, bg=bgc) 
        fl2.place(x=0, y=50)
        label2 = Label(fl2, text=str(P3.PROBLEM_MISC), font=textFont, bg=bgc, padx=170)
        label2.pack(fill=BOTH, expand=1)

    if cat == "End":
        qb.destroy()
        winTitle.destroy()
        qb = Button(qbf, text="Exit", font=optionFont, bg=brown,fg=white, 
                    activebackground=actb, activeforeground=actf,
                    command=lambda: quit())
        qb.pack(fill=BOTH, expand=1)

        
        label1 = Label(back, text="Game Over", font=textFont, bg=bgc)
        label1.pack()
        
        fl2 = Frame(height=int(HEIGHT/1000*450), width=int(HEIGHT/1000*800), master=back, bg=bgc) 
        fl2.place(x=0, y=50)
        label2 = Label(fl2, text=Current_State.goal_message(), font=textFont, bg=bgc, padx=10)
        label2.pack(fill=BOTH, expand=1)
...
def nextTurn(s):
    ...
    if True:#is_goal(s):        
        #End
        enf = Frame(height=HEIGHT,width=HEIGHT,master=mw)
        enf.pack_propagate(0)
        enf.pack()
        enb = Button(enf, text="Game Over\n" + Current_State.goal_message() , font = buttonFont,
                      activebackground=actb, activeforeground=actf,
                      command=lambda: quit())
        enf.place(x=0,y=0)
        enb.pack(fill=BOTH,expand=1)
